...
FixMUI::FixMUI(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg)
{
    // if (narg != 7) error->all(FLERR,"Illegal fix mui/meso command");
...
void FixMUI::post_integrate()
{
    ...
    for (int i = 0; i < nlocal; i++) 
    {
        if ( (mask[i] & groupbit) && pushregion->match(x[i][0], x[i][1], x[i][2]) )
        {
			...
			if (logfile)
                fprintf(logfile, "point location %f %f %f, velocity is %f \n", x[i][0], x[i][1], x[i][2], v[i][0]);
        }
    }
    ...
    interface->barrier( time - 1);
        ...
        fprintf(logfile, "<<< debug >>> after barrier\n");
